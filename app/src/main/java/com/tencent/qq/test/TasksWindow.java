...
public class TasksWindow {
    ...
    private static View init(Context context) {
        if (infoView == null) {
            synchronized (TasksWindow.class) {
                if (infoView == null) {
                    if (context == null) return null;
                    sWindowManager = (WindowManager) context.getApplicationContext().getSystemService(Context
                            .WINDOW_SERVICE);
                    sWindowParams = new WindowManager.LayoutParams(
                            ...
                            Build.VERSION.SDK_INT <= Build.VERSION_CODES.N ? WindowManager.LayoutParams.TYPE_TOAST :
                                    WindowManager.LayoutParams.TYPE_PHONE, 0x18,
    ...
    public static void show(Context context, String text) {
        ...
        TextView tv_name = infoView.findViewById(R.id.tv_name);
